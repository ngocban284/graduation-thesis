...
library Math {
    ...
    uint256 constant Zp = 2736030358979909402780800718157159386076813972158567259200215660948447373041;

    function computeLagrangeCoefficient(uint8[] memory _listIndex) internal pure returns (uint256[] memory) {
        ...
        for (uint8 indexI; indexI < _listIndex.length; indexI++) {
            ...
            lagrangeCoefficient[indexI] = mulmod(numerator, denominator, CurveBabyJubJub.JUB_SUBORDER);
        }
    ...
    function inverse(uint256 val) internal pure returns (uint256 invVal) {
        ...
        while (newR != 0) {
            ...
            (t, newT) = (
                ...
                addmod(
                    t,
                    (CurveBabyJubJub.JUB_SUBORDER - mulmod(q, newT, CurveBabyJubJub.JUB_SUBORDER)),
