...
export default class ReedSolomonDecoder {
  ...
  public decode(
    received: Int32Array,
    twoS: number /*int*/
  ): void /*throws ReedSolomonException*/ {
    const field = this.field;
    const poly = new GenericGFPoly(field, received);
    const syndromeCoefficients = new Int32Array(twoS);
    let noError: boolean = true;
    for (let i = 0; i < twoS; i++) {
      const evalResult = poly.evaluateAt(
    ...
    const sigmaOmega = this.runEuclideanAlgorithm(
    ...
    for (let i = 0; i < errorLocations.length; i++) {
      ...
      if (position < 0) {
        throw new Exception(
          Exception.ReedSolomonException,
          "Bad error location"
        );
      }
  ...
  private runEuclideanAlgorithm(
    ...
    b: GenericGFPoly,
    R: number /*int*/
  ): GenericGFPoly[] /*throws ReedSolomonException*/ {
  ...
  ): Int32Array /*throws ReedSolomonException*/ {
